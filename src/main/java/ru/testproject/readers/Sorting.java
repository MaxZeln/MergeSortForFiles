...
public class Sorting {
    public static void mergeSortInteger(int[] a, int n, String order) {
        ...
        mergeInteger(a, l, r, mid, n - mid, order);
    }

    public static void mergeInteger(int[] a, int[] l, int[] r,
                                    int left, int right, String order) {

        int i = 0, j = 0, k = 0;
        while (i < left && j < right) {
            if (order.equals("-a")) //сортировка по возрастанию
            {
                if (l[i] <= r[j]) {
                    a[k++] = l[i++];
                } else {
            ...
            } else if (order.equals("-d")) { //сортировка по убыванию
                if (l[i] >= r[j]) {
                    a[k++] = l[i++];
                } else {
                    a[k++] = r[j++];
